...
class WorkOrder(models.Model):
    ...
    def __str__(self):
        return (
            f"OT-{self.id} ({self.get_order_type_display()}) para {self.vehicle.plate}"
        )
    ...
    def recalculate_costs(self):
        ...
        parts_total = self.parts_used.aggregate(
            total=Sum(
                ExpressionWrapper(
                    F("quantity") * F("cost_at_moment"), output_field=DecimalField()
                )
            )
        )["total"]
...
class WorkOrderPart(models.Model):
    work_order = models.ForeignKey(
        WorkOrder, on_delete=models.CASCADE, related_name="parts_used"
    )
