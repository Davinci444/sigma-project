...
def vehicle_costs_report(request):
    ...
    for vehicle in vehicles:
        costs = WorkOrder.objects.filter(vehicle=vehicle, status=WorkOrder.OrderStatus.VERIFIED).aggregate(
        ...
        writer.writerow([vehicle.plate, vehicle.brand, vehicle.model, vehicle.year, cost_internal, cost_external, cost_parts, total_general])
    return response
...
def preventive_compliance_report(request):
    response = HttpResponse(
        content_type='text/csv',
        headers={'Content-Disposition': f'attachment; filename="reporte_cumplimiento_preventivos_{timezone.now().strftime("%Y-%m-%d")}.csv"'},
    )
    response.write(u'\ufeff'.encode('utf8'))
    writer = csv.writer(response)
    writer.writerow([
        'Placa', 'Zona Actual', 'KM Actual', 'Estado Odómetro', 'Manual Asignado',
        'KM Último Preventivo', 'Próximo Hito (KM)', 'Descripción Próxima Tarea',
        'KM para Próximo Servicio', 'Estado de Cumplimiento'
    ])
